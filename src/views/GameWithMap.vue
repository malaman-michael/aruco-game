...
// Canvas offscreen per il rendering della mappa (una sola volta)
const offscreenMapCanvas = document.createElement('canvas')
const offscreenCtx = offscreenMapCanvas.getContext('2d')
...
function updateCanvasSize() {
  ...
  // Se le dimensioni del canvas offscreen cambiano, forziamo il ridisegno
...
function drawMapOffscreen() {
  ...
  // Se le dimensioni non corrispondono, aggiorna la griglia ed esci (il prossimo frame riproverà)
  if (map.cols !== gridCols || map.rows !== gridRows) {
    gameStore.setGridSize(map.cols, map.rows)
    return
  }

  // Chiave per la cache: cambia quando mappa, omografia o dimensioni cambiano
  ...
  for (let row = 0; row < gridRows; row++) {
    for (let col = 0; col < gridCols; col++) {
      ...
      fontSize = Math.min(48, Math.max(16, fontSize))
...
function drawMapOverlay() {
  ...
  // Disegna la mappa offscreen solo se sporca
  drawMapOffscreen()
...
watch(() => [gameStore.gridCols, gameStore.gridRows], () => {
  markMapDirty()
  requestRedraw()
})
