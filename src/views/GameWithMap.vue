  ...
  <div class="game-view">
      ...
      <div class="corner-indicator">
        <div class="corner-box">
          ...
          <div class="corner-dots-indicator">
            <span class="indicator-dot" :class="{ active: visibleAnchorsCount >= 1 }">1</span>
            <span class="indicator-dot" :class="{ active: visibleAnchorsCount >= 2 }">2</span>
            <span class="indicator-dot" :class="{ active: visibleAnchorsCount >= 3 }">3</span>
            <span class="indicator-dot" :class="{ active: gameStore.homographyReady }">✓</span>
          </div>
          <span class="corner-count">{{ visibleAnchorsCount }} / 3+</span>
        </div>
      </div>
    ...
    <div v-if="viewMode === 'camera'" class="status-bar" role="status" aria-live="polite" aria-atomic="true">
      <span v-if="!gameStore.homographyReady" class="status-calibrating">
        📍 Ancore visibili: {{ visibleAnchorsCount }}
        <span class="corner-dots">
          <span class="corner-dot" :class="{ acquired: visibleAnchorsCount >= 1 }">1</span>
          <span class="corner-dot" :class="{ acquired: visibleAnchorsCount >= 2 }">2</span>
          <span class="corner-dot" :class="{ acquired: visibleAnchorsCount >= 3 }">3</span>
        </span>
        (servono 3+)
      </span>
...
function selectMap(mapId) {
  ...
  if (mapId) {
    ...
    if (map) {
      ...
      gameStore.setGridSize(map.cols, map.rows)
      // Reset homography per evitare che vecchie ancore sporchino il conteggio
...
function toggleVoice() {
  ...
  if (voice.enabled) {
    ...
    setTimeout(() => {
      voice.announceAnchorsCount?.(visibleAnchorsCount.value)
    }, 800)
  }
}
...
function onFrameProcessed(payload) {
  // Aggiornamenti gestiti da CameraView, ma possiamo usarlo per debug
}
...
// Quando cambia la mappa selezionata, reset homography
