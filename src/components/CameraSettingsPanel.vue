<!-- src/components/CameraSettingsPanel.vue -->
<template>
  <Teleport to="body">
        ...
        <div class="settings-panel">
          ...
          <div class="section">
            ...
            <div class="toggle-row">
              ...
              <button class="toggle-btn" :class="{ active: cam.grayscale }" @click="cam.grayscale = !cam.grayscale; cam.save()">
                {{ cam.grayscale ? 'ON' : 'OFF' }}
              </button>
            </div>
            <SliderRow label="Soglia binarizzazione" :value="cam.threshold" :min="0" :max="255" unit="" hint="0 = disabilitata" @update="v => { cam.threshold = v; cam.save() }" />
          ...
          <div class="section">
            ...
            <div class="toggle-row">
              ...
              <button class="toggle-btn" :class="{ active: cam.showGrid }" @click="cam.showGrid = !cam.showGrid; cam.save()">
                {{ cam.showGrid ? 'ON' : 'OFF' }}
              </button>
            </div>
            <SliderRow label="Opacità griglia" :value="Math.round(cam.gridOpacity * 100)" :min="10" :max="100" unit="%" @update="v => { cam.gridOpacity = v / 100; cam.save() }" />
            <div class="toggle-row">
              <span class="toggle-label">ID sui marker</span>
              <button class="toggle-btn" :class="{ active: cam.showIds }" @click="cam.showIds = !cam.showIds; cam.save()">
                {{ cam.showIds ? 'ON' : 'OFF' }}
              </button>
            </div>
            <div class="toggle-row">
              <span class="toggle-label">Effetto cubo 3D</span>
              <button class="toggle-btn" :class="{ active: cam.showCubes }" @click="cam.showCubes = !cam.showCubes; cam.save()">
          ...
          <div class="section">
            ...
            <div class="toggle-row">
              ...
              <button class="toggle-btn" :class="{ active: gameStore.freeMode }" @click="gameStore.toggleFreeMode()">
